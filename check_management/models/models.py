...
class PaymentCheckLine(models.Model):
    ...
    def _onchange_currency(self):
        for rec in self:
            if rec.currency_id:
                company = rec.payment_id.company_id
                if not company:
                    raise UserError("Company is not set. Please ensure the payment is linked to a company.")
                balance = rec.currency_id._convert(
                    rec.amount_currency, company.currency_id, company,
                    rec.payment_id.date or fields.Date.context_today(rec)
                )
                rec.check_amount = balance if balance > 0.0 else 0.0
            else:
