...
class CheckDeduct(models.TransientModel):
    ...
    def action_deduct(self):
        ...
        if self.credit_journal_id.currency_id:
            move_line_1['currency_id'] = self.credit_journal_id.currency_id.id
            move_line_2['currency_id'] = self.credit_journal_id.currency_id.id
            move_line_1['amount_currency'] = (check_amount * -1)
            move_line_2['amount_currency'] = check_amount
            move_line_1['credit'] = (check_amount / self.credit_journal_id.currency_id.rate)
            move_line_2['debit'] = (check_amount / self.credit_journal_id.currency_id.rate)
...
class CheckCash(models.TransientModel):
    _name = 'check.cash'
    _description = 'Cash Journals'

    credit_journal_id = fields.Many2one("account.journal", string="Debit Journal", required=True)
    # credit_journal_id = fields.Many2one("account.journal", string="Credit Journal")
    date = fields.Date(required=True, default=fields.Date.context_today)
    ...
    def action_cash(self):
        ...
        if self.credit_journal_id.currency_id:
            currency_id = self.credit_journal_id.currency_id.id
            move_line_1.update({
                'currency_id': currency_id,
                'amount_currency': check_amount,
                'debit': check_amount * self.credit_journal_id.currency_id.rate,
            })
            move_line_2.update({
                'currency_id': currency_id,
                'amount_currency': -check_amount,
                'credit': check_amount * self.credit_journal_id.currency_id.rate,
            })
